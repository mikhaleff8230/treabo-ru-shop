  ...
  return (
    <div className="flex w-full flex-col items-start">
      {data?.map((item: any, index) => {
        // Используем перевод статуса, если он есть, иначе используем name
        const statusKey = item?.status || item?.name?.toLowerCase().replace(/\s+/g, '-');
        const translatedName = statusKey ? t(statusKey, { defaultValue: item?.name }) : item?.name;
        ...
        return (
          ...
          >
            ...
            >
              {item && (
                <span className="text-13px font-normal">{translatedName}</span>
              )}
            </div>
          </div>
        );
