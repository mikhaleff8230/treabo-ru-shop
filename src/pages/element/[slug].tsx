...
const ProductPage: NextPageWithLayout<InferGetServerSidePropsType<typeof getServerSideProps>> = ({ product, meta }) => {
  ...
  // ВАЖНО: canonical URL должен быть в формате /element/{full_slug} (с 12-значным кодом) БЕЗ языкового префикса
  ...
  if (!canonicalUrl || canonicalUrl === 'https://treabo.md' || canonicalUrl === 'https://sancan.rv' || !canonicalUrl.includes('/element/')) {
    // Приоритет: используем canonical_url из product, затем full_slug, затем slug-id
    const productCanonical = (product as any)?.canonical_url;
    const productFullSlug = (product as any)?.full_slug;
    
    if (productCanonical && productCanonical.includes('/element/')) {
      // Используем canonical_url из API
      canonicalUrl = productCanonical;
    } else if (productFullSlug) {
      // Используем full_slug из API (уже содержит код)
      canonicalUrl = `${baseUrl}/element/${productFullSlug}`;
    } else if (slug) {
      // Fallback: используем slug из URL (уже содержит код для новых товаров)
    ...
    <>
      ...
      <div className="relative">
        <div className="h-full min-h-screen p-4 md:px-6 lg:px-8 lg:pt-6">
          ...
          {/* Специальная страница для черновика/архива */}
          {isHiddenStatus ? (
            <>
              {/* Заголовок статуса */}
              <div className="mb-6">
