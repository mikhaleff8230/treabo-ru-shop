...
export interface PlaceVideo {
  id: string;
  url: string; // Оригинальное видео
  preview?: string; // 3-секундное превью
  preview_url?: string; // Альтернативное название для preview
  poster?: string; // Постер из первого кадра
  poster_url?: string; // Альтернативное название для poster
  thumbnail?: string; // Thumbnail для списков
  thumbnail_url?: string; // Альтернативное название для thumbnail
  duration?: number; // Длительность в секундах
  formatted_duration?: string; // Читаемый формат (например, "1:23")
  width?: number;
  height?: number;
  file_size?: number;
  formatted_file_size?: string; // Читаемый формат (например, "2.5 MB")
  mime_type?: string;
  has_optimized_versions?: boolean;
}
...
export interface PlacePaginator extends PaginatorInfo<Place> {}
