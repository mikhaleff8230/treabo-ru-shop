...
export function useProducts(
  ...
  return {
    products: Array.isArray(data?.pages) ? data.pages.flatMap((page) => page.data) : [],
...
export function useRelatedProducts(slug: string, limit: number = 8) {
  const { locale } = useRouter();
  ...
  const {
    ...
    [API_ENDPOINTS.PRODUCTS_RELATED, slug, locale, limit],
    () => client.products.related({ slug, language: locale, limit }),
    {
      enabled: !!slug,
