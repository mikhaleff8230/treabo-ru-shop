...
export function useOrders(options?: OrderQueryOptions) {
  const {
    ...
    ({ pageParam }) => {
      const page = (pageParam as { page?: number })?.page ?? 1;
      return client.orders.all(Object.assign({}, options, { page }));
    },
    {
...
export function useDownloadableProductOrders(options?: OrderQueryOptions) {
  ...
  const {
    ...
    ({ pageParam }) => {
      const page = (pageParam as { page?: number })?.page ?? 1;
      return client.orders.downloadable(Object.assign({}, formattedOptions, { page }));
    },
    {
      getNextPageParam: ({ current_page, last_page }) =>
        last_page > current_page ? { page: current_page + 1 } : undefined,
  ...
  // Безопасная обработка данных из infinite query (как в useProducts)
  const downloadableFiles = Array.isArray(data?.pages)
    ? data.pages.flatMap((page) => page?.data ?? [])
    : [];

  // Отладка для проверки структуры данных
  if (typeof window !== 'undefined' && data?.pages) {
    console.log('useDownloadableProductOrders: pages count:', data.pages.length);
    console.log('useDownloadableProductOrders: first page:', data.pages[0]);
    console.log('useDownloadableProductOrders: downloadableFiles count:', downloadableFiles.length);
    if (downloadableFiles.length > 0) {
      console.log('useDownloadableProductOrders: first file:', downloadableFiles[0]);
    }
  }
